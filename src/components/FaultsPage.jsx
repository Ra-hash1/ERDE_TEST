...
import { AlertCircle, ArrowLeft, CheckCircle, AlertTriangle, Zap } from 'lucide-react';
...
function FaultsPage({ user }) {
  ...
  useEffect(() => {
    const fetchData = async (isInitial = false) => {
      try {
        ...
        const mockFaults = faultParameters.reduce((acc, fault) => {
          acc[fault] = Math.random() > 0.7 ? 'Active' : 'Inactive';
          return acc;
        }, {});
        ...
        setError(`Failed to fetch faults data for ${selectedVehicle}`);
        if (isInitial) {
          setLoading(false);
        }
      }
  ...
  const activeFaults = Object.entries(faults).filter(([_, status]) => status === 'Active');
        ...
        <div className="relative group">
          ...
          <div className="relative bg-gradient-to-br from-gray-800 via-gray-700 to-gray-900 rounded-3xl border-2 border-purple-500/30 shadow-2xl p-6 backdrop-blur-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(faults).map(([key, status], index) => (
